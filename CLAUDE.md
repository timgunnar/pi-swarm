...
pi-swarm — 树莓派集群分布式大模型推理管理系统。控制面/数据面分离架构，对标工业 K8s 部署实践。
...
├─ Agent 分布式引擎   ← Scatter/Gather  │  所有 Pi 角色完全相同
├─ Vue 3 Frontend    ← Dashboard        │  可随时加入/退出
├─ CLI (k3s-pi)      ← 命令行工具
...
- Agent 分布式执行引擎 (Scatter/Gather) — 大任务拆分为子任务并行分发到多 Pi，结果聚合。Claude Code 连接单一 API 地址即可将子 Agent 推理分发到不同 Pi
- 数据库 SQLite 起步 + SQLAlchemy + Alembic — 后续可平滑迁移 PostgreSQL

## Full Design & Plan

- 设计文档: `docs/superpowers/specs/2026-05-31-pi-swarm-design.md`
- 实施计划: `docs/superpowers/plans/2026-05-31-pi-swarm-plan.md`
...
k3s-pi agent run --task "分析代码质量" --workers 3   # Scatter/Gather 分布式执行
k3s-pi status
...
```
backend/     FastAPI — app/main.py 入口, api/v1/ 路由(含 agent/), engines/ 抽象, agent/ Scatter/Gather, services/ 逻辑, k8s/ 客户端
